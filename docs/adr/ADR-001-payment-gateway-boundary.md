...
- **Reversal is implemented.** `reverse_transaction` posts the fee REVERSAL (sign-flipped, `posting_type="reversal"`, `reverses_post_id` = the original fee post), transitions `settled → refunded` (rows_affected-gated), and emits `GatewayTransactionRefunded` carrying `payment_entry_id` so the composition ACL reverses the PaymentEntry via `payment.reverse_payment` (→ `PaymentCancelled` → billing restores outstanding). All-or-nothing + idempotent. Proven at the unit level by `ggc6_fee_reversal` (the pure reversal composer: sign-flip, balanced, links the original).
