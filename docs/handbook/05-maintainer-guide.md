...
metaphor make entity Vendor --module payment-gateway
...
metaphor schema schema validate payment-gateway

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate payment-gateway --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor payment-gateway
...
> `payment-gateway` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate payment-gateway`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> payment-gateway` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration payment-gateway --database-url …`).
4. Regenerate code: `metaphor schema schema generate payment-gateway --target all --force`.
