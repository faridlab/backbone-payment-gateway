...
metaphor migration generate Invoice payment-gateway
metaphor migration run
...
(`payment-gateway` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed payment-gateway          # run Rust seeders
metaphor migration generate-seeds payment-gateway  # emit SQL seed files
```
