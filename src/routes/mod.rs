...
/// Create stateless routes for the PaymentGateway module.
///
...
///
/// ```ignore
/// let module = PaymentGatewayModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::PaymentGatewayModule) -> Router<()> {
    Router::new()
        .merge(create_gateway_transaction_routes(module.gateway_transaction_service.clone()))
        .merge(create_payment_gateway_provider_routes(module.payment_gateway_provider_service.clone()))
}

/// Read-only routes for the PaymentGateway module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_payment-gateway_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_payment-gateway_routes(module: &crate::PaymentGatewayModule) -> Router<()> {
    Router::new()
        .merge(create_gateway_transaction_read_routes(module.gateway_transaction_service.clone()))
        .merge(create_payment_gateway_provider_read_routes(module.payment_gateway_provider_service.clone()))
}

/// Get all routes (stateless) for the PaymentGateway module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::PaymentGatewayModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the PaymentGateway module.
///
...
pub fn create_combined_routes(module: &crate::PaymentGatewayModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the PaymentGateway module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::PaymentGatewayModule) -> Router<AppState> {
    Router::new()
